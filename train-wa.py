...
if NUM_ADV_EPOCHS > 0:
    ...
    logger.log(
        "Standard Accuracy-\tTest: {:2f}%.".format(trainer.eval(test_dataloader) * 100)
    )
...
if args.resume_path:
    start_epoch = (
        trainer.load_model_resume(os.path.join(args.resume_path, "state-last.pt")) + 1
    )
...
for epoch in range(start_epoch, NUM_ADV_EPOCHS + 1):
    ...
    if epoch % 400 == 0:
        shutil.copyfile(
            WEIGHTS, os.path.join(LOG_DIR, f"weights-best-epoch{str(epoch)}.pt")
        )
